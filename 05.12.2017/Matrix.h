...
void reshuffleSquares(int matrix[][N], int row, int column);

bool checkingForSquareMatrix(int row, int column);
...
void reshuffleSquares(int matrix[][N], int row, int column)
{
	if (checkingForSquareMatrix(row, column))
	{
		for (int i = 0; i < column / 2; ++i)
		{
			for (int j = 0; j < row; ++j)
			{
				if (j < row / 2)
				{
					swap(matrix[i][j], matrix[i + column / 2][j + row / 2]);
				}
				else
				{
					swap(matrix[i][j], matrix[i + column / 2][j - row / 2]);
